...
│   ├── processed/cleaned/                 # 清洗后最终语料（161 篇，其中 160 篇有效入库）
...
> 依赖清单：`requests`、`beautifulsoup4`、`pymupdf`、`numpy`、`sentence-transformers`、`torch`、`flask`、`waitress`。
...
自检输出示例：`向量形状: (3, 512)`、`相似度自检通过 [PASS]`，即环境就绪。本地模型缓存存在时会强制离线加载。
...
> **说明**：本项目正式交付自研 DC-RAG 链路；检索环节完全本地（BGE 嵌入 + 层次化知识库），仅生成环节调用 `.env` 中配置的兼容 LLM 接口。

```bash
# 1. 环境配置见上方"环境配置"章节（依赖安装 + .env 填写）
# 2. 启动服务
python app/server.py
# 3. 浏览器访问 http://127.0.0.1:8000，可选择政策问答、材料摘要、报告草稿或情报简报

# Windows/内网生产入口（配合反向代理、认证与 HTTPS）
...
```bash
python scripts/export_dataset.py                  # 导出文档备份（data/export/documents.jsonl）
python scripts/export_dataset.py --format chunks  # 导出文本块备份（data/export/chunks_backup.jsonl）
```

一键爬取 + 清洗 + 入知识库（手动执行，不自动调度、不执行 git 操作）：

```bash
python scripts/auto_update.py                # 一键：爬取 → 清洗 → 摘要 → 切分 → 聚类 → 重建知识库 → 验证检索
python scripts/auto_update.py --dry-run      # 仅预览候选，不下载不清洗
python scripts/auto_update.py --skip-crawl   # 跳过爬取，仅清洗+更新知识库
python scripts/auto_update.py --skip-kb      # 仅爬取+清洗，不更新知识库
# 也可作为代码接口调用：from scripts.auto_update import run_pipeline; run_pipeline()
```

> 流水线将新数据完整送入知识库（新增文档生成摘要、重建三级索引）并可被 DC-RAG 检索到；未变化文本块复用旧向量，仅编码新增/变化块；残缺文件自动跳过。
> 运行后输出新增/残缺提醒，由人工审核后再手动 `git add/commit/push`，脚本不会自动推送。
